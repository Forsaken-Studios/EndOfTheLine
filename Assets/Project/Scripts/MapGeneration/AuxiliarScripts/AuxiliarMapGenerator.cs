...
public static class AuxiliarMapGenerator
{
    ...
    // Given a subgrid it inserts it in a copy of the original grid.
    ...
    //    List<Cell> auxRoomCells = new List<Cell>();

    //    int rows = roomGrid.GetLength(0);
    //    int cols = roomGrid.GetLength(1);

    //    Debug.Log($"MAP GENERATION - Creating copied grid. Room size: {rows}x{cols}");

    //    // Crear celdas para el nuevo grid
    ...
    //            copiedGrid[row, col] = new Cell(
    //                copiedCell.Row,
    //                copiedCell.Col,
    //                roomGrid[row - initialRow, col - initialCol].State,
    //                roomGrid[row - initialRow, col - initialCol].IsRoomPlaceable,
    //                copiedCell.ig,
    //                copiedCell.ih,
    //                roomGrid[row - initialRow, col - initialCol].EntranceDirection
    //            );

    //            Debug.Log($"MAP GENERATION - Setting cell [{row}, {col}] to state: {copiedGrid[row, col].State}");

    //            if (copiedGrid[row, col].State != CellState.Empty)
    //            {
    //                auxRoomCells.Add(copiedGrid[row, col]);
    //                Debug.Log($"MAP GENERATION - Added non-empty cell [{row}, {col}] to auxRoomCells.");
    //            }
    //        }
    //    }

    //    // Asignar padres, temporal parents y destinos desde el grid copiado
    //    Debug.Log("MAP GENERATION - Assigning parents and temporal parents.");

    //    for (int row = initialRow; row < initialRow + rows; row++)
    //    {
    //        for (int col = initialCol; col < initialCol + cols; col++)
    //        {
    //            Cell originalCell = copiedGrid[row, col];
    //            Cell newCell = copiedGrid[row, col];
    ...
    //                foreach (Cell parentCell in originalCell.Parents)
    //                {
    //                    Cell newParent = copiedGrid[parentCell.Row, parentCell.Col];
    //                    newCell.Parents.Add(newParent);
    //                }
    //            }

    //            // Copiar el TemporalParent
    //            if (originalCell.TemporalParent != null)
    //            {
    //                newCell.TemporalParent = copiedGrid[originalCell.TemporalParent.Row, originalCell.TemporalParent.Col];
    //            }

    //            // Copiar las DestinationCells
    //            if (originalCell.DestinationCells.Count > 0)
    //            {
    //                foreach (Cell destinationCell in originalCell.DestinationCells)
    //                {
    //                    Cell newDestination = copiedGrid[destinationCell.Row, destinationCell.Col];
    //                    newCell.DestinationCells.Add(newDestination);
    //                }
    //            }
    //        }
    ...
    //    Debug.Log("MAP GENERATION - Finished assigning parents and destinations.");

    //    List<Cell> copiedRoomCells = new List<Cell>();

    //    foreach (Cell auxCell in auxRoomCells)
    //    {
    //        copiedRoomCells.Add(copiedGrid[auxCell.Row, auxCell.Col]);
    //        Debug.Log($"MAP GENERATION - Copied grid state for cell [{auxCell.Row}, {auxCell.Col}]: {copiedGrid[auxCell.Row, auxCell.Col].State}");
    //    }

    //    // Intentar recalcular los caminos
    //    Debug.Log("MAP GENERATION - Recalculating paths (ReFindPath).");
    //    Cell[,] modifiedGrid = ReFindPath(copiedRoomCells, copiedGrid);
    //    if (modifiedGrid != null)
    //    {
    //        copiedGrid = modifiedGrid;
